<script lang="ts">
    ...
    import { branding, authProviders } from '$lib/stores/tenant';
    ...
    // Reactive auth method checks from tenant config
    $: showGoogle = $authProviders?.google ?? true;
    $: showEmail = $authProviders?.email ?? true;
...
<div class="login-container">
    <div class="login-card">
        <div class="logo-section">
            {#if $branding?.logo}
                <img src={$branding.logo} alt={$branding?.logo_alt || 'Logo'} class="logo" />
            {:else}
                <img src="/cogzy-logo.svg" alt="Cogzy" class="logo" />
            {/if}
            <h1>{$branding?.title || 'Cogzy'}</h1>
            <p class="tagline">{$branding?.tagline || 'Your cognitive companion'}</p>
        </div>
