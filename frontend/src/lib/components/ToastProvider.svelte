<script lang="ts" context="module">
    ...
    export const toast = {
        show(category: PhraseCategory, message: string, duration = 3000) {
            const id = crypto.randomUUID();
            toasts.update(t => [...t, { id, category, message, duration }]);
        ...
        error(message: string) {
            return this.show('error', message, 5000);
        },
        remove(id: string) {
...
<div class="toast-container">
    {#each $toasts as t (t.id)}
        <div
            class="toast toast-{t.category}"
            in:fly={{ y: 50, duration: 200 }}
            out:fade={{ duration: 150 }}
            animate:flip={{ duration: 200 }}
        >
            <span class="toast-icon">{icons[t.category]}</span>
