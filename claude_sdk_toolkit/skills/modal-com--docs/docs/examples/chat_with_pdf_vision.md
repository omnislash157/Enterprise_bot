---
title: Chat with PDF: RAG with ColQwen2
...
That’s what enables Modal Functions to automatically scale up and down [based on the number of incoming requests](https://modal.com/docs/guide/cold-start).

In this example, we use a [`modal.Dict`](https://modal.com/docs/guide/dicts-and-queues) to store state information between Function calls.
...
Otherwise, the model weights will be downloaded on the first query. For more on storing model weights on Modal, see [this guide](https://modal.com/docs/guide/model-weights).
...
[Chat with PDF: RAG with ColQwen2](#chat-with-pdf-rag-with-colqwen2)[Setup](#setup)[Setting up dependencies](#setting-up-dependencies)[Specifying the ColQwen2 model](#specifying-the-colqwen2-model)[Caching the model weights](#caching-the-model-weights)[Defining a Chat with PDF service](#defining-a-chat-with-pdf-service)[Loading PDFs as images](#loading-pdfs-as-images)[Chatting with a PDF from the terminal](#chatting-with-a-pdf-from-the-terminal)[A hosted Gradio interface](#a-hosted-gradio-interface)[Addenda](#addenda)
