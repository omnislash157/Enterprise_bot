---
title: Stream transcriptions with Kyutai STT
url: https://modal.com/docs/examples/streaming_kyutai_stt
type: examples
---
...
[View on GitHub](https://github.com/modal-labs/modal-examples/blob/main/06_gpu_and_ml/speech-to-text/streaming_kyutai_stt.py)
...
Then we define a Modal App and an [Image](https://modal.com/docs/guide/images) with the dependencies of our speech-to-text system.
...
For more details on patterns for handling model weights on Modal, see [this guide](https://modal.com/docs/guide/model-weights).
...
We use a Modal [Cls](https://modal.com/docs/guide/lifecycle-functions) so that we can separate out the model loading and setup code from the inference.

For more on lifecycle management with Clses and cold start penalty reduction on Modal, see [this guide](https://modal.com/docs/guide/cold-start).
...
and via a Modal [Queue](https://modal.com/docs/guide/queues) for Python clients.
...
[Stream transcriptions with Kyutai STT](#stream-transcriptions-with-kyutai-stt)[Setup](#setup)[Run a local Python client to test streaming STT](#run-a-local-python-client-to-test-streaming-stt)[Deploy a streaming STT service on the Web](#deploy-a-streaming-stt-service-on-the-web)
