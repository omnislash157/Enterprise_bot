---
title: Fine
url: https://modal.com/docs/examples/fine_tune_asr
type: examples
---
...
[View on GitHub](https://github.com/modal-labs/modal-examples/blob/main/06_gpu_and_ml/openai_whisper/fine_tune_asr.py)
...
We’ll use the “small” subset of “Science and Technology” from the [GigaSpeech](https://huggingface.co/datasets/speechcolab/gigaspeech) dataset, which is enough data to see the model improve on scientific terms in just a
few epochs.

Note: GigaSpeech is a [gated model](https://huggingface.co/docs/hub/en/models-gated),
so you’ll need to accept the terms on the [dataset card](https://huggingface.co/datasets/speechcolab/gigaspeech) and create a [Hugging Face Secret](https://modal.com/secrets/) to download it.
...
We define the environment where our functions will run by building up a base [container `Image`](https://modal.com/docs/guide/images) with our dependencies using `Image.uv_pip_install`. We also set environment variables
...
We use [Modal Volumes](https://modal.com/docs/guide/volumes) for data we want to persist across function calls. In this case, we’ll create a cache
...
We can define both our inference function and an endpoint using a Modal [Cls](https://modal.com/docs/reference/modal.Cls).
This will allow us to take advantage of [lifecycle hooks](https://modal.com/docs/guide/lifecycle-functions) to load the model just once on container startup using the `@modal.enter` decorator.
We can use [modal.fastapi\_endpoint](https://modal.com/docs/reference/modal.fastapi_endpoint) to expose our inference function as a web endpoint.
...
[Fine-tune Whisper to Improve Transcription on Domain-Specific Vocab](#fine-tune-whisper-to-improve-transcription-on-domain-specific-vocab)[Setup](#setup)[Set up the container image](#set-up-the-container-image)[Training](#training)[Defining our training Function](#defining-our-training-function)[Deploying our fine-tuned model for inference](#deploying-our-fine-tuned-model-for-inference)[Support code](#support-code)
...
```shiki
modal run 06_gpu_and_ml/openai_whisper/fine_tune_asr.py --test
```
