---
title: Serve a Discord Bot on Modal
url: https://modal.com/docs/examples/discord_bot
type: examples
---
...
Here, we set up a simple [FastAPI app](https://fastapi.tiangolo.com/) to run that service and deploy it easily Modal’s [`@asgi_app`](https://modal.com/docs/guide/webhooks#serving-asgi-and-wsgi-apps) decorator.
...
First, we define the [container image](https://modal.com/docs/guide/images) that all the pieces of our bot will run in.
...
For more details on serving Python web apps on Modal, see [this guide](https://modal.com/docs/guide/webhooks).
...
[Set up our App and its Image](#set-up-our-app-and-its-image)[Hit the Free Public APIs API](#hit-the-free-public-apis-api)[Set up a Discord app](#set-up-a-discord-app)[Register a Slash Command](#register-a-slash-command)[Finish setting up Discord bot](#finish-setting-up-discord-bot)
