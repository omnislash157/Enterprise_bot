---
title: Train a model to solve coding problems using GRPO and TRL
...
[View on GitHub](https://github.com/modal-labs/modal-examples/blob/main/06_gpu_and_ml/reinforcement-learning/grpo_trl.py)
...
This example demonstrates how to run [GRPO](https://arxiv.org/pdf/2402.03300) on Modal using the TRL [GRPO trainer](https://huggingface.co/docs/trl/main/en/grpo_trainer) GRPO is a reinforcement learning algorithm introduced by DeepSeek, and was used to train DeepSeek R1.
...
Once you have the model checkpoints in your Modal Volume, you can load the weights and perform inference using vLLM. For more on storing model weights on Modal, see [this guide](https://modal.com/docs/guide/model-weights).
...
[Train a model to solve coding problems using GRPO and TRL](#train-a-model-to-solve-coding-problems-using-grpo-and-trl)[Defining the reward function](#defining-the-reward-function)[Kicking off a training run](#kicking-off-a-training-run)[Speeding up training with vLLM](#speeding-up-training-with-vllm)[Performing inference on the trained model](#performing-inference-on-the-trained-model)
