---
title: Run a FastRTC app on Modal
url: https://modal.com/docs/examples/fastrtc_flip_webcam
type: examples
---
...
You can try it out [here](https://modal-labs-examples--example-fastrtc-flip-webcam-ui.modal.run/) or just dive straight into the code to run it yourself.

First, we import the `modal` SDK
and use it to define a [container image](https://modal.com/docs/guide/images) with FastRTC and related dependencies.
...
so we can host it on Modal’s cloud platform using the [`modal.asgi_app` decorator](https://modal.com/docs/guide/webhooks#serving-asgi-and-wsgi-apps) with [Modal Function](https://modal.com/docs/guide/apps).
