---
title: Fold proteins with Boltz
url: https://modal.com/docs/examples/boltz_predict
type: examples
---

# Fold proteins with Boltz

---

[View on GitHub](https://github.com/modal-labs/modal-examples/blob/main/06_gpu_and_ml/protein-folding/boltz_predict.py)
...
of a [protein-ligand complex](https://github.com/jwohlwend/boltz/blob/main/examples/affinity.yaml)*
...
You can pass any options for the [`boltz predict` command line tool](https://github.com/jwohlwend/boltz/blob/main/docs/prediction.md) as a string, like
...
Code running on Modal runs inside containers built from [container images](https://modal.com/docs/guide/images) that include that code’s dependencies.
...
Note that the `boltz` command-line tool we use takes the path to a [specially-formatted YAML file](https://github.com/jwohlwend/boltz/blob/main/docs/prediction.md#yaml-format) that includes definitions of molecules to predict the structures of and optionally paths to [Multiple Sequence Alignment](https://en.wikipedia.org/wiki/Multiple_sequence_alignment) (MSA) files
...
Here’s the implementation of that function. For details, see our [guide to storing model weights on Modal](https://modal.com/docs/guide/model-weights).
...
We package the outputs into a tarball which contains the predicted structure as a [Crystallographic Information File](https://en.wikipedia.org/wiki/Crystallographic_Information_File) and the binding affinity as a JSON file.
...
[Fold proteins with Boltz-2](#fold-proteins-with-boltz-2)[Setup](#setup)[Fold a protein from the command line](#fold-a-protein-from-the-command-line)[Addenda](#addenda)
