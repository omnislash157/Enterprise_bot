---
title: Run a job queue that turns documents into structured data with Datalab Marker
...
For the purpose of this tutorial, we’ve also built a [React + FastAPI web app on Modal](https://modal.com/docs/examples/doc_ocr_webapp) that works together with it, but note that you don’t need a web app running on Modal
...
We also define the dependencies we need by specifying an [Image](https://modal.com/docs/guide/images).
...
So instead, we create a Modal [Volume](https://modal.com/docs/guide/volumes) to store the models. Each Modal Volume is a durable filesystem that any Modal Function can access.
...
Using the [`@app.function`](https://modal.com/docs/reference/modal.App#function) decorator, we set up a Modal [Function](https://modal.com/docs/reference/modal.Function).
...
by adding a [`local_entrypoint`](https://modal.com/docs/reference/modal.App#local_entrypoint) that invokes the Function `.remote`ly.
...
app, take a look at the [receipt parser frontend](https://modal.com/docs/examples/doc_ocr_webapp) tutorial.

[Run a job queue that turns documents into structured data with Datalab Marker](#run-a-job-queue-that-turns-documents-into-structured-data-with-datalab-marker)[Define an App](#define-an-app)[Testing and debugging remote code](#testing-and-debugging-remote-code)[Deploying the document conversion service](#deploying-the-document-conversion-service)
