---
title: Real
url: https://modal.com/docs/examples/webrtc_yolo
type: examples
---
...
If you just want to quickly get started with WebRTC for a small internal service or a hack project, check out [our FastRTC example](https://modal.com/docs/examples/fastrtc_flip_webcam) instead.
...
It handles passing messages between the client peer and the signaling server using a [WebSocket](https://modal.com/docs/guide/webhooks#websockets) for persistent, bidirectional communication over the Web within a single Function Call.
...
we’ll write out the processed video stream for testing. For more on storing model weights on Modal, see [this guide](https://modal.com/docs/guide/model-weights).
...
If you want to use it, you’ll need to create an account [here](https://dashboard.metered.ca/login?tool=turnserver) and then create a Modal [Secret](https://modal.com/docs/guide/secrets) called `turn-credentials` [here](https://modal.com/secrets).
...
[Real-time object detection with WebRTC and YOLO](#real-time-object-detection-with-webrtc-and-yolo)[What is WebRTC?](#what-is-webrtc)[What makes up a WebRTC application?](#what-makes-up-a-webrtc-application)[Setup](#setup)[Cache weights and compute graphs on a Volume](#cache-weights-and-compute-graphs-on-a-volume)[Implement a SignalingServer](#implement-a-signalingserver)[Addenda](#addenda)[YOLO helper functions](#yolo-helper-functions)
